...
const findAlternateText = (alternateTexts: LyricAlternateText[] | undefined, role: Exclude<SubtitleContentMode, 'none'>): string | null =>
	alternateTexts?.find(entry => entry.role === role && entry.text.trim())?.text.trim() ?? null

export const resolveLyricAlternateText = (source: AlternateTextSource | null | undefined, mode: SubtitleContentMode): string | null => {
	if (!source || mode === 'none') {
		return null
	}
...
export const resolveSubtitleContentMode = (mode: SubtitleContentMode | undefined, legacyShowTranslation = true): SubtitleContentMode =>
	mode ?? (legacyShowTranslation ? 'translation' : 'none')
