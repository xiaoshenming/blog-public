...
import type { TtmlAgent, TtmlBackgroundVocal, TtmlLyricLine, TtmlSubLyricContent, TtmlSyllable, TTMLResult } from './ttmlParser'
...
const isWesternJoinableSyllable = (text: string): boolean => {
	...
	return trimmed.length > 0 && !NON_WESTERN_TEXT_REGEX.test(trimmed) && WESTERN_JOINABLE_TEXT_REGEX.test(trimmed)
}
...
const buildWordsFromSyllables = (
	...
	for (let index = 0; index < converted.length; index += 1) {
		...
		const shouldMergeWithNext = Boolean(next && !current.endsWithSpace && isWesternJoinableSyllable(current.text) && isWesternJoinableSyllable(next.text))
...
const convertAlternateTexts = (role: 'translation' | 'romanization', entries: TtmlSubLyricContent[] | undefined): LyricAlternateText[] => {
	if (!entries?.length) {
		return []
	}
...
const getFirstText = (entries: TtmlSubLyricContent[] | undefined): string | undefined => entries?.find(entry => entry.text.trim().length > 0)?.text

const isChorusSongPart = (songPart: string | undefined): boolean => songPart?.trim().toLowerCase() === 'chorus'

const convertBackgroundVocal = (vocal: TtmlBackgroundVocal | undefined, buildTimedWords: TimedWordBuilder): LyricBackgroundVocal | undefined => {
	...
	const alternateTexts = [...convertAlternateTexts('translation', vocal.translations), ...convertAlternateTexts('romanization', vocal.romanizations)]
...
const convertLine = (line: TtmlLyricLine, buildTimedWords: TimedWordBuilder): Line => {
	...
	const alternateTexts = [...convertAlternateTexts('translation', line.translations), ...convertAlternateTexts('romanization', line.romanizations)]
...
const isSameTtmlBlock = (mainLine: Line, candidate: Line): boolean =>
	mainLine.blockIndex === undefined || candidate.blockIndex === undefined || mainLine.blockIndex === candidate.blockIndex
...
const foldAgentOverlapHarmonyLines = (lines: Line[]): Line[] => {
	...
	for (const line of lines) {
		const mainLine = [...mainLines].reverse().find(candidate => isAgentOverlapHarmony(candidate, line))
		...
		const inferredVocals = [lineToBackgroundVocal(line), ...(line.backgroundVocals ?? (line.backgroundVocal ? [line.backgroundVocal] : []))]
		const existingVocals = mainLine.backgroundVocals ?? (mainLine.backgroundVocal ? [mainLine.backgroundVocal] : [])
...
export const buildLyricDataFromTTMLResult = (result: TTMLResult, buildTimedWords: TimedWordBuilder): LyricData => {
	const convertedAgents = convertAgents(result.metadata.agents)?.agents
	const lines = foldAgentOverlapHarmonyLines(result.lines.map(line => convertLine(line, buildTimedWords)))
	const ttml = {
