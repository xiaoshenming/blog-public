import type { ExplicitFileTimedLyricFormat, NonTtmlTimedLyricFormat, TimedLyricFormat } from './types'

export type { ExplicitFileTimedLyricFormat, NonTtmlTimedLyricFormat, TimedLyricFormat } from './types'
...
export function detectTimedLyricFormat(content?: string): TimedLyricFormat {
	...
	if (normalized.startsWith('WEBVTT') || normalized.includes('-->') || VTT_TIMING_LINE_REGEX.test(normalized)) {
		return 'vtt'
	}

	if (ENHANCED_LRC_ANGLE_REGEX.test(normalized) || ENHANCED_LRC_INLINE_BRACKET_REGEX.test(normalized)) {
		return 'enhanced-lrc'
	}
