...
// src/app/music/visualizer/harmonyRuntime.ts
...
export const getLyricsBackgroundVocals = (lines: Line[]): LyricBackgroundVocal[] =>
	lines.flatMap(line => getLineBackgroundVocals(line)).sort((left, right) => left.startTime - right.startTime || left.endTime - right.endTime)

export const resolveHarmonyAlternateText = (
	vocal: LyricBackgroundVocal,
	subtitleContentMode: SubtitleContentMode | undefined,
	legacyShowTranslation = true
): string | null => resolveLyricAlternateText(vocal, resolveSubtitleContentMode(subtitleContentMode, legacyShowTranslation))
...
export const resolveHarmonySnapshotFromVocals = (vocals: LyricBackgroundVocal[], currentTime: number): HarmonySnapshot => {
	const lines = vocals
	...
	const signature = lines.map(entry => `${entry.key}:${entry.tokens.map(token => token.status).join(',')}`).join('|')

	return { signature, lines }
}

export const resolveHarmonySnapshot = (line: Line | null | undefined, currentTime: number): HarmonySnapshot =>
	resolveHarmonySnapshotFromVocals(getLineBackgroundVocals(line), currentTime)
