...
// src/app/music/visualizer/songHandover.ts
...
/**
 * Fold of the word segmentation over the set, mixed into the signature below.
 *
 * A segmentation rewrites the lines in place: same count, same text, only `wordSegments`
 * added. Without this the signature could not see that, the gate answered `idle`, and the new
 * split did not reach the screen until the song changed.
 *
 * Hashing has to describe the split itself, not just how many lines carry one: re-running the
 * segmenter or hand-editing a line leaves the coverage count untouched while changing what gets
 * drawn. Boundary LENGTHS are enough, since the boundaries always join back to the line's own text.
 *
 * Numeric FNV-1a rather than the string key in lyrics/wordSegmentation.ts because this runs over
 * every line on every render of the modes that use it, and must not build a string per pass.
 * Lyrics with no split cost one null check per line and always fold to the same constant.
 */
...
): boolean => (sawPlaybackReset && playbackTime >= instrumentalCommitSeconds) || elapsedMs >= readyGraceMs
...
export const useVisualizerSongCommit = ({
	...
	const [committed, setCommitted] = useState<VisualizerSongCommit>(() => ({ seed, lines, isInstrumental: false }))
	...
	}, [committedSeed, committedSignature, currentTime, instrumentalCommitSeconds, isCommittedInstrumental, lyricsSignature, readyGraceMs, seed])
