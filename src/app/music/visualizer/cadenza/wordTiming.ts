...
import { getLineTransitionTiming } from '../lyrics/renderHints'
import type { Line, Word as WordType } from '../types'
import type { ResolvedLineRenderTiming } from './types'
import { clamp, easeInOutQuad, easeOutCubic, mix } from './textUtils'

// src/app/music/visualizer/cadenza/wordTiming.ts
// Per-word timeline derivation: status, reveal progress, the classic glow/body envelopes and
// the line-level enter/exit envelope. Everything here is a pure function of (time, line, word).

export const getWordStatus = (time: number, lineTiming: ResolvedLineRenderTiming, word: WordType) => {
	const lookahead = lineTiming.wordRevealMode === 'fast' ? 0.045 : lineTiming.wordRevealMode === 'instant' ? 0 : 0.18
	const activeEndTime = lineTiming.wordRevealMode === 'instant' ? lineTiming.lineRenderEndTime : word.endTime
...
export const getWordProgress = (time: number, wordRevealMode: ResolvedLineRenderTiming['wordRevealMode'], word: WordType) => {
...
export const getClassicCharGlow = (time: number, word: WordType, glyphIndex: number, glyphCount: number, wordGraphemeTimings: GraphemeTiming[] = []) => {
	const duration = Math.max(word.endTime - word.startTime, 0.1)
...
export const getClassicLineEnvelope = (time: number, line: Line | null, lineTiming: ResolvedLineRenderTiming) => {
	if (!line) {
		return {
			opacity: 1,
			scale: 1,
...
export const resolveLineRenderTiming = (line: Line): ResolvedLineRenderTiming => {
	...
	const transitionTiming = getLineTransitionTiming(rawDuration, renderHints?.lineTransitionMode ?? 'normal', wordRevealMode)
