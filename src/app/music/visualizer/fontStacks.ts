...
// src/app/music/visualizer/fontStacks.ts
// Resolves the CSS font stack and weight a theme asks for. Ported as-is from upstream
// src/utils/fontStacks.ts: it is self-contained and every mode reads fonts through it.
...
export const resolveThemeFontWeight = (theme: Pick<Theme, 'fontWeight'> | null | undefined, fallback: number) =>
...
const TRANSLATION_FONT_STACKS: Record<Theme['fontStyle'], string> = {
	...
	serif: `${SUGAR_SERIF_FAMILY}, "Iowan Old Style", Georgia, "Times New Roman", "Noto Serif CJK SC", "Source Han Serif SC", "Songti SC", "STSong", "SimSun", "Noto Serif JP", "Source Han Serif JP", "Yu Mincho", "MS PMincho", serif`,
...
const formatFontFamily = (fontFamily: string) => {
	...
	return CSS_GENERIC_FONT_FAMILIES.has(normalized.toLowerCase()) ? normalized : quoteFontFamily(normalized)
}
...
const buildCustomFontFamilyStack = (theme: Pick<Theme, 'fontFamily' | 'fontFamilyStack'>) => {
	const familiesToNormalize: (string | null | undefined)[] = [...(theme.fontFamily ? [theme.fontFamily] : []), ...(theme.fontFamilyStack ?? [])]
