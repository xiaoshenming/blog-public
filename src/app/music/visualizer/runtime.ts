...
export const getUpcomingLine = (lines: Line[], currentLineIndex: number, currentTime: number): Line | null => {
...
export const shouldPreheatLine = (line: Line | null | undefined, currentTime: number, window: VisualizerPreheatWindow): boolean => {
	if (!line) {
		return false
	}
...
export const useVisualizerRuntime = ({ currentTime, currentLineIndex, lines, getLineEndTime }: UseVisualizerRuntimeOptions) => {
	...
	const upcomingLine = useMemo(() => getUpcomingLine(lines, currentLineIndex, currentTimeValue), [currentLineIndex, currentTimeValue, lines])

	const nextLines = useMemo(() => getUpcomingLines(lines, currentLineIndex, 2), [currentLineIndex, lines])
