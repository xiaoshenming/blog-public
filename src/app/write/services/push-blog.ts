...
export async function pushBlog(params: PushBlogParams): Promise<void> {
	...
	if (!form?.slug) throw new Error(t('write.slugRequired'))

	if (mode === 'edit' && originalSlug && originalSlug !== form.slug) {
		throw new Error(t('write.slugImmutable'))
	}
	...
	const commitMessage = mode === 'edit' ? t('write.commitUpdate', { slug: form.slug }) : t('write.commitCreate', { slug: form.slug })
	...
	toast.info(t('write.preparingFiles'))
	...
	if (allLocalImages.length > 0) {
		toast.info(t('write.uploadingImages'))
	...
	toast.info(t('write.creatingFiles'))
	...
	// create tree
	toast.info(t('write.creatingTree'))
